...
public class LuckyCharm extends PerkBasic {

    public LuckyCharm(UUID uuid,Integer innerLevel){
        super(uuid, PerkType.LuckyCharm,innerLevel);
    }
    ...
    protected void onKill(PlayerKillEvent e) {
        ...
        if(MathUtils.Chance(i)){
            getPlayer().sendMessage("§e+1颗金苹果!");
            getPlayer().getInventory().addItem(new ItemBuilder(Material.GOLDEN_APPLE).toItemStack());
        }
    }
