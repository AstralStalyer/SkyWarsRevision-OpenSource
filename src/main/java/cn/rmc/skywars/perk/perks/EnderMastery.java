...
public class EnderMastery extends PerkBasic {

    public EnderMastery(UUID uuid, Integer innerLevel){
        super(uuid, PerkType.EnderMastery,innerLevel);
    }
    ...
    protected void onEntityDamaged(EntityDamageByEntityEvent e) {
        if(e.getDamager().getType() != EntityType.ENDER_PEARL) return;
        e.setDamage(e.getDamage() * (1-0.2*getInnerLevel()));
