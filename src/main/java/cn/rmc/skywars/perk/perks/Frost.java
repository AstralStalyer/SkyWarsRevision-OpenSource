...
public class Frost extends PerkBasic {

    public Frost(UUID uuid,Integer innerLevel){
        super(uuid, PerkType.Frost,innerLevel);
    }
        ...
        for(Player player: PlayerListener.lastarrow.keySet()){
            if(PlayerListener.lastarrow.get(player).equals(e.getEntity().getUniqueId())){
                if(MathUtils.Chance(getInnerLevel())){
                    player.addPotionEffect(new PotionEffect(PotionEffectType.SLOW,20*3,0),true);
                }
            }
            break;
        }
